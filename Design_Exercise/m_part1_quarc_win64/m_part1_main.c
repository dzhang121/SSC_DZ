 ...
 *
 * Date:           Fri Feb 17 14:42:58 2023
 * Model version:  5.8
 * Matlab version: 9.5 (R2021a) 14-Nov-2020
 ****************************************************************************/
...
typedef RT_MODEL_m_part1_T RT_MODEL;

/*====================*
 * External functions *
 *====================*/
EXTERN RT_MODEL * m_part1(void);
  ...
  _do_assertion(const char * expression, const char * file_name, int line_number)
{
  ...
  rtmSetErrorStatus(m_part1_M, GBLbuf.message);
}
  ...
  control_c_handler(int signal_number)
{
  ...
  GBLbuf.stopExecutionFlag = 1;
  rtExtModeStart();
  hil_task_stop(m_part1_DW.HILReadEncoderTimebase_Task)
    ;
}
  ...
  main(int argc, char * argv[])
{
  ...
  rtExtModeQuarcParseArgs(argc, (const char **) argv, "shmem://m_part1:1");
  ...
  if (parseError) {
    (void) fprintf(stderr,
                   "\nUsage: m_part1 -option1 val1 -option2 val2 -option3 ...\n\n");
    (void) fprintf(stderr,
  ...
  if (rtExtModeQuarcStartup(rtmGetRTWExtModeInfo(S),
    ...
    if (rtmGetErrorStatus(S) == NULL) {
      ...
      fflush(stdout);
      result = hil_task_start(m_part1_DW.HILReadEncoderTimebase_Task, (t_clock)
        m_part1_P.HILReadEncoderTimebase_Clock, 1000.0, -1)
        ;
      if (result == 0) {
